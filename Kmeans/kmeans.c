...
void processkMeans(data_struct *data_in, data_struct *clusters, double *newCentroids, double* SumOfDist)
{
	int i, j, k;
	double tmp_dist = 0;
	int tmp_index = 0;
	...
	for (i = 0; i < data_in->secondary_dim; i++)
	{
		...
		{
			tmp_dist = calculateEuclideanDistance(dataset + i * data_in->leading_dim, centroids + k * clusters->leading_dim, data_in->leading_dim);
